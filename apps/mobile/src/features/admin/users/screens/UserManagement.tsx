...
export default function UserManagement() {
  ...
  const openEdit = (user: AdminUser) => router.push({ pathname: "/admin/UpdateUser", params: { id: user._id } });
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.pageHeader}><Pressable onPress={() => router.back()} style={styles.pageBack}><Ionicons name="arrow-back" size={22} color="#FFF" /></Pressable><Text style={styles.heading}>User Management</Text></View>
      ...
      <Modal visible={!!editing} transparent animationType="slide" onRequestClose={closeEdit}>
        <KeyboardAvoidingView style={[styles.overlay, Platform.OS === "android" && { paddingBottom: keyboardHeight }]} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <Pressable style={styles.backdrop} onPress={closeEdit} />
          <View style={[styles.modal, keyboardHeight > 0 && { maxHeight: screenHeight - keyboardHeight - 16 }]}>
            ...
            <ScrollView ref={editScrollRef} style={styles.formScroll} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" contentContainerStyle={styles.modalContent}>
              {([ ["name", "Full name", "person-outline"], ["username", "Username", "at-outline"], ["email", "Email", "mail-outline"], ["password", "New password", "lock-closed-outline"] ] as const).map(([key, label, icon], index) => (
                <View key={key} style={styles.fieldGroup}><Text style={styles.label}>{label}{key !== "password" ? " *" : " (optional)"}</Text><View style={styles.fieldWrap}><Ionicons name={icon} size={18} color="#98A2B3" /><TextInput value={form[key]} onChangeText={(value) => setForm((current) => ({ ...current, [key]: value }))} onFocus={() => requestAnimationFrame(() => editScrollRef.current?.scrollTo({ y: index * 82, animated: true }))} placeholder={label} placeholderTextColor="#667085" secureTextEntry={key === "password"} keyboardType={key === "email" ? "email-address" : "default"} autoCapitalize={key === "email" || key === "username" ? "none" : "words"} style={styles.field} /></View></View>
              ))}
